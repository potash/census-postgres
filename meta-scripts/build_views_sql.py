...
def run(data_root, working_dir, config):
    ...
    cell_names = []
    for table_id, rows in groupby(sqn_lookup_file, key=lambda row: row['Table ID'] if row['File ID'] else None):
        for row in rows:
            sqn = int(row[sqn_col_name])
            line_number = row[line_no_col_name]

            if not table_id or not line_number or line_number.endswith('.5') or line_number == '.':
