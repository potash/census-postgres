/*
--RUN ONCE ONLY!!! 
CREATE SCHEMA acs2013_5yr;
*/

SET search_path = acs2013_5yr, public;
...
--The data here can is derived from the document: 
--http://www2.census.gov/acs2013_5yr/summaryfile/ACS_2013_SF_Tech_Doc.pdf
--Am a little unclear where the information about the sumlevels comes from (there is similar
--information in Appendix B of that document, but its not a straight forward conversion to 
--what appears here), but note that the "Descr" and "sumlevels" fields serve only as metadata
--and don't need to be populated correctly for the db to build properly
...
INSERT INTO geoheader_schema (name, descr, field_size, starting_position, sumlevels) VALUES ('state', 'State (FIPS Code)', 2, 26, '040, 050, 060, 067, 070, 140, 150, 155, 160, 170, 172, 230, 260, 269, 270, 280, 283, 286, 290, 311, 312, 313, 315, 316, 320, 321, 322, 323, 324, 331, 333, 336, 338, 340, 341, 345, 346, 351, 352, 353, 354, 356, 357, 358, 360, 361, 362, 363, 364, 365, 366, 500, 510, 550, 610, 612, 620, 622, 795, 950, 960, 970');
INSERT INTO geoheader_schema (name, descr, field_size, starting_position, sumlevels) VALUES ('county', 'County of current residence', 3, 28, '050, 060, 067, 070, 140, 150, 155, 270, 313, 316, 322, 324, 353, 354, 357, 358, 362, 363, 365, 366, 510, 612, 622');
INSERT INTO geoheader_schema (name, descr, field_size, starting_position, sumlevels) VALUES ('cousub', 'County Subdivision (FIPS)', 5, 31, '060, 067, 070, 354, 358, 363, 366');
INSERT INTO geoheader_schema (name, descr, field_size, starting_position, sumlevels) VALUES ('place', 'Place (FIPS Code)', 5, 36, '070, 155, 160, 172, 269, 312, 321, 352, 361');
INSERT INTO geoheader_schema (name, descr, field_size, starting_position, sumlevels) VALUES ('tract', 'Census Tract', 6, 41, '140, 150');
...
CREATE TABLE data_dictionary (
	...
	Table_ID varchar, 
	Sequence_Number int, 
	Line_Number double precision, 
	Start_Position int, 
	Total_Cells_in_Table varchar, 
	Total_Cells_in_Sequence int, 
	Table_Title varchar, 
	Subject_Area varchar
);

--Will fail if table structure doesn't match imported file
--Note that the following command needs to be run on the downloaded data dictionary
--with bash to get rid of white space in order for the function below to work properly
--this is because the format of the data dictionary changed slightly after 2011:
--sed -i='original' -r 's/,\s+/,/g' "$d_dict_file_path"
