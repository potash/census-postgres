INSERT INTO acs2008_3yr.geoheader SELECT
	...
	NULLIF(btrim(substring(all_fields from 136 for 2)), '') AS submcd,
	NULLIF(btrim(substring(all_fields from 138 for 5)), '') AS sdelm,
	NULLIF(btrim(substring(all_fields from 143 for 5)), '') AS sdsec,
	NULLIF(btrim(substring(all_fields from 148 for 5)), '') AS sduni,
	NULLIF(btrim(substring(all_fields from 153 for 1)), '') AS ur,
	NULLIF(btrim(substring(all_fields from 154 for 1)), '') AS pci,
	NULLIF(btrim(substring(all_fields from 155 for 6)), '') AS blank5,
	NULLIF(btrim(substring(all_fields from 161 for 5)), '') AS blank6,
	NULLIF(btrim(substring(all_fields from 166 for 5)), '') AS puma5,
	NULLIF(btrim(substring(all_fields from 171 for 5)), '') AS blank7,
	NULLIF(btrim(substring(all_fields from 176 for 40)), '') AS geoid,
	NULLIF(btrim(substring(all_fields from 216 for 200)), '') AS name,
	NULLIF(btrim(substring(all_fields from 418 for 50)), '') AS blank8
FROM acs2008_3yr.tmp_geoheader;
